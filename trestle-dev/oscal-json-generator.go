...
type ImplementedRequirement struct {
	...
	TestProcedures []TestProcedure `json:"test-procedures"`
}
...
func main() {
	...
	// Marshal to JSON
	output, err := json.MarshalIndent(&oscal, "", "  ")
	if err != nil {
		log.Fatalf("error: %v", err)
	}

	// Write to JSON file
	err = ioutil.WriteFile("oscal/CCC.OS.oscal.json", output, 0644)
	if err != nil {
		log.Fatalf("error: %v", err)
	}

	fmt.Println("OSCAL component definition JSON generated successfully!")
}
